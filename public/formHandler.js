...
function showModal(html, type='info', title='Notification', footerHTML='<button class="modal-btn-primary" onclick="closeModal()">OK</button>') {
  ...
  const cont = m.querySelector('.modal-container');
  cont.className = 'modal-container ' + type;
...
function manageOversize(i) {
  ...
  showModal(
    `<p><strong>${f.name}</strong><br>Size: ${fmt(f.size)} (limit 10MB)</p>
...
function openCompression(){
  showModal(
    `<p>Select an online service; after compressing, re-upload.</p>
...
document.addEventListener('DOMContentLoaded',()=>{
  const inp=qs('#upload'), form=qs('#feedbackForm'), sb=qs('#submitBtn'),c=qs('#consentCheckbox');
  c.onchange=()=>{sb.disabled=!c.checked; sb.classList.toggle('disabled',!c.checked)};
  qs('#phone').oninput=e=>{const pe=qs('#phone-error'); if(!e.target.value) return pe.style.display='none'; pe.textContent=validPhone(e.target.value)?'':'Invalid number'; pe.style.display=validPhone(e.target.value)?'none':'block';};
  inp.onchange=e=>{const nf=[...e.target.files].filter(f=>!selectedFiles.find(x=>x.name===f.name)); if(selectedFiles.length+nf.length>10){nf.splice(10-selectedFiles.length); showModal('Max 10 files; extras ignored.','warning','Limit')} selectedFiles=[...selectedFiles,...nf]; e.target.value=''; preview()};
  form.onsubmit=e=>{
    e.preventDefault();
    ...
    showModal('All set! Click Confirm to submit.','confirm','Confirm Submission',
      `<button class="modal-btn-secondary" onclick="closeModal()">Cancel</button>
